    ...
    for (size_t k = 0; k != nz - 1; ++k) {
        for (size_t j = 0; j != ny - 1; ++j) {
            ...
            for (size_t i = xl; i != xr; ++i) {
                ...
                curTriCounter += numTris[caseId];

                const bool *isCutCur = isCut[caseId]; // size 12

                ge0.xstart += isCutCur[0];
                ge0.ystart += isCutCur[3];
                ge0.zstart += isCutCur[8];
                ...
                if (isXEnd) {
                    ge0.ystart += isCutCur[1];
                    ge0.zstart += isCutCur[9];
                }
                if (isYEnd) {
                    ge1.xstart += isCutCur[2];
                    ge1.zstart += isCutCur[10];
                }
                if (isZEnd) {
                    ge2.xstart += isCutCur[4];
                    ge2.ystart += isCutCur[7];
                }

                if (isXEnd and isYEnd) {
                    ge1.zstart += isCutCur[11];
                }
                if (isXEnd and isZEnd) {
                    ge2.ystart += isCutCur[5];
                }
                if (isYEnd and isZEnd) {
                    ge3.xstart += isCutCur[6];
                }
            }
        }
    }
...
bool FlyingEdgesSerial::isCutEdge(
...
inline uchar
FlyingEdgesSerial::calcCaseEdge(
        bool const &prevEdge,
...
inline uchar
FlyingEdgesSerial::calcCubeCase(
...
inline void
FlyingEdgesSerial::calcTrimValues(
        size_t &xl, size_t &xr,
    ...
    xl = std::min(ge0.xl, std::min(ge1.xl, std::min(ge2.xl, ge3.xl)));
    xr = std::max(ge0.xl, std::max(ge1.xl, std::max(ge2.xl, ge3.xl)));
...
FlyingEdgesSerial::interpolateOnCube(
        cube_t const &pts,
...
FlyingEdgesSerial::writeObj() {
    ...
    for (size_t i = 0; i < normals.size(); ++i) {
        file << "vn " << normals[i][0] << " " << normals[i][1] << " " << normals[i][2] << std::endl;
    }
    ...
    for (size_t i = 0; i < tris.size(); ++i) {
        file << "f "
             << tris[i][0] + 1 << "//" << tris[i][0] + 1 << " "
             << tris[i][1] + 1 << "//" << tris[i][1] + 1 << " "
             << tris[i][2] + 1 << "//" << tris[i][2] + 1 << std::endl;
    }
